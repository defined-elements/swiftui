...
///
/// It is accessible from outside, but it is not designed to be initialized by users.
/// The only purpose of it is to hold the view and Modifier pattern on `DefinedViewModifier` design pattern.
///
/// - Note:
///   It only supports `DefinedView` as its root.
...
public struct DefinedViewModifiedView<Root> : DefinedView where Root: DefinedView {
    ...
    ///
    /// It is generally made for filtering `DefinedView` when your `DefinedViewModifier` is designed for some specific root views.
    ...
    var configuration: DefinedViewModifiedViewConfiguration
    
    /// A function containing the build process for view hierarchy.
    var content: (Root) -> AnyView
    
    /// A constructor taking the `(Root) -> AnyView` function from another `DefinedViewModifiedView`.
    ///
    /// - Note:
    ///   This constructor is not accessible from outside.
    ///
    /// - Parameters:
    ...
    /// A constructor building for another modified level at `DefinedViewModifier` design pattern.
    ///
    /// - Note:
    ///   This constructor is not accessible from outside.
    ///
    ...
    init<Content>(root: Root,
                  configuration: DefinedViewModifiedViewConfiguration,
                  @ViewBuilder content: @escaping (Root) -> Content) where Content: View {
        self.root = root
        self.configuration = configuration
